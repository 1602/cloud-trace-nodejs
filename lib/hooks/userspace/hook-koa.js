...
async function middleware(ctx, next) {
  ...
  const traceHeader = agent.parseContextFromHeader(
    ctx.request.headers[constants.TRACE_CONTEXT_HEADER_NAME]) || {};
  if (!agent.shouldTrace(ctx.request.url, traceHeader.options)) {
    return await next();
  }
  ...
  namespace.run(function() {
    const rootContext = startRootSpanForRequest(ctx.request, traceHeader);
    const context = agent.generateTraceContext(rootContext, true);
    if (context) {
      ctx.headers[constants.TRACE_CONTEXT_HEADER_NAME] = context;
    } else {
...
function startRootSpanForRequest(req, traceHeader) {
  ...
  rootContext.addLabel(TraceLabels.HTTP_SOURCE_IP, req.ip);
  return rootContext;
}
